...
class TextDataset(Dataset):
    ...
    def __getitem__(self, item: int) -> Tuple[torch.Tensor, int]:
        ...
        # cut seq if it is big
        if len(indices) > self.max_length - 2:
            indices = indices[:self.max_length-2]
            length = self.max_length
        # add BOS и EOS tokens
        indices = [self.bos_id] + indices + [self.eos_id]
        # pad seq if it is short
        if len(indices) > self.max_length:
            # it's not possible, but I'm too lazy to rewrite conditions
            pass
        else:
            pad_size = self.max_length - len(indices)
            indices.extend([self.pad_id] * pad_size)
            length = len(indices) - pad_size
